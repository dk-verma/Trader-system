...
void writeSock( int sock, char *mesg )
{
    int n = write( sock, mesg, strlen(mesg) );
    if (n < 0) {
         perror("ERROR writing in to socket");
...
int auth_Valid( char *user, char *pass, int *UserNum )
{
...
// swap requests
void Swap_reqs(struct request *a, struct request *b) {
   struct request temp;
   temp = *a;
   *a = *b;
   *b = temp;
...
void sort_Insert(struct request req) {
   struct request *array = ( (req.type=='B')?(buyQueue[req.itemNumber]):(sellQueue[req.itemNumber]) );
   ...
   if(*head == *tail) {
      ...
      *tail = (*tail + 1)%500;
      ...
      int idx = *head, i;
      while(!llessthanr(req, array[idx]) && idx != *tail) {
         idx = (idx+1)%500;
      }
...
int memsubstr(const char *buffer, int len, const char *sequence) {
   int i;
   for(i=0; i+strlen(sequence) < len ; i++) {
      if(memcmp(buffer + i, sequence, strlen(sequence)) == 0) {
         return 1;
      }
   }
   return 0;
...
int main( int argc, char *argv[] ) {
   ...
   */
   ...
   while(1)
   {
        ...
        while(1) {
            memset(buff, '\0', 256);
            int sz = read( newsockfd, buff, 256);
            if(sz < 0) {
    ...
    if(strcmp(msg[2], "LOGIN")==0)
    {
        ...
        {
            case 1:     
                writeSock( newsockfd, "SUCCESS\nLOG IN Successful!\n") ;
                break;
            case 2:     
                writeSock(newsockfd, "FAIL\nWrong Password!!!\nTry again\n");
                break;
            case 3: 
                writeSock(newsockfd, "FAIL\nWrong username!!!\nTry again\n");
                break;
